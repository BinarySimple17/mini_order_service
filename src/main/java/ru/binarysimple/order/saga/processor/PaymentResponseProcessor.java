...
public class PaymentResponseProcessor implements EventProcessor<SagaEvents.PaymentResponseEvent>{
    ...
    public void processEvent(SagaEvents.PaymentResponseEvent event) {
        ...
        if (event.getSuccess()) {
            ...
            notificationService.sendNotification(saga, orderMapper.toOrderResultDto(order), EventType.PAYMENT_COMPLETED);
        } else {
...
//            saga.setErrorMessage("Payment failed: " + event.getMessage());
            sagaRepository.save(saga);
            notificationService.sendNotification(saga, orderMapper.toOrderResultDto(order), EventType.PAYMENT_FAILED);
        }
    }
}
